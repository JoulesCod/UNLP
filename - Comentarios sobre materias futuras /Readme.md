### Información de contacto (IC): https://ic.info.unlp.edu.ar/informacion-de-contacto/

## Comentarios materias 
> (Comentarios que me dieron personas que ya cursaron estas materias)
# 3er AÑO
## 1er Cuatri

### Electrónica y Electrotecnia  (EE) --> 
- Profe (Pablo) de teoría exigente,
- IR LOS JUEVES A LA MAÑANA CON PABLO.
- ir a las prácticas.
- seguir bien la clase,
- CARPETA EN PAPEL, puede que la pidan MUY COMPLETA 
- estudiar la práctica, adicionales y parciales viejos (todo)
- Parcial 2hs, 3 puntos, 40% bien, hay que justificar con mucha teoría, hay que saber mucho.
- 3 fechas 1er modulo, 3 fechas 2do modulo. --> la 3ra fecha (flotante) del 1er módulo coincide con la 1era fecha del 2do módulo 

### Introducción a Diseño Lógico (IDL) --> 
- A GIO NO LE GUSTA.
- hacer circuitos lógicos,
- estilo CAC / FAC.
- prácticas no tan necesarias y difíciles.
- TP entregables A PATO NO LE IMPORTE JUSTIFICACION.
- parcial igual todos los años, te queda esa nota. final igual q parcial. 

### Probabilidades y Estadística --> 
- fácil,
- **Melisa Magini** HACER CON ELLA SI O SI. 

### Conceptos de Bases de Datos (CBD) --> 
- cátedra de base de datos,
- 3 módulos, árboles (en pascal), archivos, hashing en dibujos.
- FINAL MUCHO MAS FACIL Q PROMO.
- IR SOLO A PRACTICAS,
- VER VIDEOS

## 2do Cuatri

### Introducción al Procesamiento de Señales (IPS) --> 
- la práctica es re cualquiera.
- MATERIA MAS DIFICIL

### Redes de Datos 1 (RD1) -->
- esta PATO piola,
- fácil,
- ir a teorías

### Taller de Arquitectura (TA) -->
- 2 proyectos (similares q años pasados) + 3 patcialitos siempre los mismos

### Ingeniería de Sofware (ing S) -->
- medio complicada.
- PROYECTO EN GRUPO.
- El proyecto lo haces en el lenguaje q quieras,
    - para eso en primer semestre de 3ro hacer una optativa de algún lenguaje PHP o punto net (haces apps y no toma parcial).
- proyecto, parcial teórico y parcial practico

# 4to AÑO
## 1er cuatri 
## 2do cuatri 
### Base de datos 
- FINAL FACIL.
- Ir solo a prácticas, VER VIDEOS
